...
gcloud config set compute/region {{{project\_0.default\_region | REGION}}}

2.  To create a variable for region, run the following command:

export REGION={{{project\_0.default\_region | REGION}}}

3.  Run the following command to create a variable for the zone:

export ZONE={{{project\_0.default\_zone | Zone}}}
...
3.  In the **Machine configuration**:
    ...
    | **Region**       |             | For more information about regions, see the Compute Engine guide, [Regions and Zones](https://cloud.google.com/compute/docs/zones).                                                                                                                                                           |
    | **Zone**         |             | **Note:** Remember the zone that you selected to use later. For more information about zones, see the Compute Engine guide, [Regions and Zones](https://cloud.google.com/compute/docs/zones).                                                                                                 |
    | **Series**       | `E2`        |                                                                                                                                                                                                                                                                                               |
...
1.  In Cloud Shell, run the following `gcloud` command to create a new VM instance from the command line:
    ...
    **Expected output:**
    ...
    Created \[...gcelab2\]. NAME: gcelab2 ZONE: {{{project\_0.default\_zone | Zone}}} MACHINE_TYPE: e2-medium PREEMPTIBLE: INTERNAL_IP: 10.128.0.3 EXTERNAL_IP: 34.136.51.150 STATUS: RUNNING
...
2.  To see all the defaults, run the following command:
    ...
    gcloud compute instances create --help **Note:** You can set the default region and zones that `gcloud` uses if you are always working within one region/zone and you don't want to append the `--zone` flag every time.
...
Continue your course with [Get Started with Cloud Shell and gcloud](https://google.qwiklabs.com/catalog_lab/320), or check out the following lab:

- [Provision Services with Google Cloud Marketplace](https://google.qwiklabs.com/catalog_lab/339)

### Next steps / Learn more

- For an overview of VMs, refer to the [Virtual machine instances](https://cloud.google.com/compute/docs/instances/) documentation.
- Check out how to [migrate VMs to the Google Cloud](https://cloud.google.com/migrate/).
- Learn more about [VPC networks](https://cloud.google.com/compute/docs/networking).
...
**Manual Last Updated September 15, 2025**
